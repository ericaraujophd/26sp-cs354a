...
4. Copy the `.env` file (posted on Moodle separately) into the project root.
...
8. Commit and push: `git add queries.py`, `git commit -m "Complete final project"`, `git push`.
...
# Add the .env file (download from Moodle, place in project root)
...
The video does not need to be polished — a screen recording with your voice is fine. Both partners must speak. Upload it anywhere accessible (YouTube unlisted, Google Drive, Calvin OneDrive, Vimeo) and submit the URL on Moodle.
